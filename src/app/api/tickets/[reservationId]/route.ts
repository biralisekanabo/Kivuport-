...
export async function GET(request: Request, context: { params: Promise<{ reservationId: string }> }) {
  ...
  page.drawText(pdfText(`Passager: ${[client?.prenom, client?.nom].filter(Boolean).join(" ") || "-"}`), { x: 48, y: 680, size: 12, font });
  page.drawText(pdfText(`Voyage: ${(reservation.voyage as { code_voyage?: string } | null)?.code_voyage || "-"}`), { x: 48, y: 655, size: 12, font });
  page.drawText(pdfText(`Embarquement: ${new Date(reservation.date_embarquement).toLocaleString("fr-FR")}`), { x: 48, y: 630, size: 12, font });
  page.drawText(pdfText(`Pavillon: ${(reservation.pavillon as { nom?: string } | null)?.nom || "-"}`), { x: 48, y: 605, size: 12, font });
  page.drawText(pdfText(`Montant: ${Number(reservation.prix_total || 0).toLocaleString("fr-FR")} FC`), { x: 48, y: 580, size: 12, font });
