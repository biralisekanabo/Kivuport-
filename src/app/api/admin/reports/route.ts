...
export async function GET(request: Request) {
  ...
  if (format === "pdf") {
    ...
    y -= 20;
    currentPage.drawText(pdfText(`Généré le ${today}`), { x: margin, y, size: 10, font, color: rgb(0.3, 0.3, 0.3) });
    y -= 30;

    currentPage.drawText(`Paiements : ${totalPayments}`, { x: margin, y, size: 11, font: fontBold, color: rgb(0.1, 0.5, 0.3) });
    currentPage.drawText(pdfText(`Chiffre d'affaires : ${Number(totalRevenue).toLocaleString("fr-FR", { minimumFractionDigits: 0, maximumFractionDigits: 2 })} USD`), { x: margin + 220, y, size: 11, font: fontBold, color: rgb(0.2, 0.3, 0.7) });
    y -= 20;
    ...
    for (const row of tableRows) {
      ...
      values.forEach((value, index) => {
        currentPage.drawText(pdfText(String(value).slice(0, 18)), { x: colX[index], y, size: 8, font, color: rgb(0.25, 0.25, 0.25) });
      });
      y -= 16;
    }
