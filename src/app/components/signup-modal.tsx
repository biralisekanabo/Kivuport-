...
export function SignupModal({ onClose, onLogin }: AuthModalProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (formData.name.trim().length < 2) {
      setStatus({ type: "error", message: "Veuillez saisir votre nom complet." });
      return;
    }

    if (isAdminEmail(formData.email)) {
      setStatus({ type: "error", message: "Cette adresse est réservée à l'administration." });
      return;
    }

    const phoneRegex = /^(?:(?:\+|00)?243)?(?:0)?(?:91|99|97|81)\d{7}$/;
    if (!phoneRegex.test(formData.phone.replace(/\s/g, ""))) {
      setStatus({
        type: "error",
        message: "Format de téléphone invalide (ex: +243991098942)",
      });
      return;
    }
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/50 backdrop-blur-sm overflow-y-auto"
      role="presentation"
          ...
          <form className="space-y-3" onSubmit={handleSubmit}>
            ...
            <div className="relative">
              ...
              <input
                ...
                type="tel"
                required
                placeholder="Numéro de téléphone (ex: +243991098942)"
