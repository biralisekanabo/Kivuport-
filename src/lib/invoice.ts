...
export async function createInvoicePdf(input: { reservationId: number; amount: number; currency: string; voyage: string; customer: string; paymentReference: string }) {
  ...
  page.drawText(pdfText(`Client: ${input.customer || "-"}`), { x: 48, y: 675, size: 12, font });
  page.drawText(pdfText(`Voyage: ${input.voyage}`), { x: 48, y: 650, size: 12, font });
  page.drawText(pdfText(`Montant paye: ${input.amount.toLocaleString("fr-FR")} ${input.currency}`), { x: 48, y: 600, size: 13, font: bold, color: rgb(0.04, 0.22, 0.28) });
  page.drawText(pdfText(`Reference paiement: ${input.paymentReference}`), { x: 48, y: 555, size: 11, font });
