...
export function otpEmail(code: string) {
  const content = `<p style="margin:0 0 10px;font-size:12px;font-weight:700;letter-spacing:1.4px;color:${colors.accent}">VÉRIFICATION</p><h1 style="margin:0;font-size:28px;line-height:1.2;color:${colors.ink}">Votre code de vérification</h1><p style="font-size:15px;line-height:1.7;color:${colors.muted}">Vous avez demandé la réinitialisation de votre mot de passe. Veuillez saisir le code ci-dessous :</p><table role="presentation" width="100%" cellspacing="0" cellpadding="0"><tr><td align="center" style="padding:12px 0 20px"><div style="display:inline-block;background:${colors.pale};border:2px dashed ${colors.line};border-radius:14px;padding:18px 40px;letter-spacing:8px;font-size:32px;font-weight:800;color:${colors.brand};font-family:Consolas,'Courier New',monospace">${code}</div></td></tr></table><p style="font-size:13px;color:${colors.muted};text-align:center;margin:0 0 24px">Ce code est valable pendant <strong style="color:${colors.accent}">60 secondes</strong>.</p><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#fef7f4;border:1px solid #f5d5c4;border-radius:10px"><tr><td style="padding:14px 18px;font-size:12px;color:#9a4a2e;line-height:1.5">🔒 Si vous n&apos;êtes pas à l&apos;origine de cette demande, ignorez cet email. Votre mot de passe restera inchangé.</td></tr></table>`;
  return shell("Code de vérification - KivuPort", "Votre code de vérification KivuPort est ci-dessous.", content);
}
